...
async def info(event):
    ...
    hn = await DRAGON(GetFullUserRequest(event.sender_id))
    ...
    if not input_str:
          await DRAGON.send_message(event.chat_id, text.format(hn.user.first_name, hn.user.last_name, event.sender_id, event.sender.username, sed.count, hn.about, event.sender_id))
          return
 
    input_str = event.pattern_match.group(1)
    ha = await DRAGON.get_entity(input_str)
    hu = await DRAGON(GetFullUserRequest(id=input_str))
    sedd = await DRAGON(P(user_id=input_str, offset=42, max_id=0, limit=80))
...
@DRAGON.on(events.callbackquery.CallbackQuery(data="misc"))
async def _(event):
