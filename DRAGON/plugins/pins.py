...
async def get_pinned(event):
    ...
    Ok = await DRAGON.get_messages(event.chat_id, ids=types.InputMessagePinned()) 
...
async def pin(event, perm):
    ...
    if "notify" in input_str:
       await DRAGON.pin_message(event.chat_id, msg, notify=True)
       return
...
async def unpin(event, perm):
    ...
    chat_id = (str(event.chat_id)).replace("-100", "")
    ok = await DRAGON.get_messages(event.chat_id, ids=types.InputMessagePinned())
    await DRAGON.unpin_message(event.chat_id, ok)
...
@DRAGON.on(events.callbackquery.CallbackQuery(data="pins"))
async def _(event):
