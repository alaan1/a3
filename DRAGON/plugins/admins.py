...
@DRAGON.on(events.callbackquery.CallbackQuery(data="admin"))
async def _(event):

    await event.edit(ADMIN_TEXT, buttons=[[Button.inline("« رجوع", data="help")]])

@DRAGON.on(events.callbackquery.CallbackQuery(data="play"))
async def _(event):

    await event.edit(PLAY_TEXT, buttons=[[Button.inline("« رجوع", data="help")]])

@DRAGON.on(events.NewMessage(pattern="رفع"))
@is_admin
async def promote(event, perm):
    ...
    if not input_str and not user:
        ...
        return
    sed = await DRAGON(GetFullUserRequest(id=user.sender_id or input_str))
    await DRAGON(EditAdminRequest(event.chat_id, user.sender_id or input_str, ChatAdminRights(
                    add_admins=False,
                    invite_users=True,
                    change_info=False,
                    ban_users=True,
...
@DRAGON.on(events.NewMessage(pattern="الرابط"))
