from DRAGON import DRAGON, BOT_USERNAME
...
@DRAGON.on(events.NewMessage(pattern="^[?!/]start$"))
async def start(event):
    ...
    if event.is_private:
       await event.client.send_file(event.chat_id,
        ...
        [Button.url("السورس", "https://github.com/taher-1/DRAGON-MUSIC")],
...
@DRAGON.on(events.callbackquery.CallbackQuery(data="start"))
async def _(event):
    if event.is_private:
       await event.edit(PM_START_TEXT.format(event.sender.first_name), buttons=[
        [Button.url("➕ اضغط هنا لاضافتي", f"https://t.me/{BOT_USERNAME}?startgroup=true")],
        [Button.url("السورس", "https://t.me/DRACULA_2022/1861")],
        [Button.url("الدعم", f"https://t.me/{Config.SUPPORT}"), Button.url("القناة", f"https://t.me/{Config.CHANNEL}")],
        [Button.inline("الاوامر", data="help")]])
       return
