...
async def lock(event, perm):
    ...
    if "الرسائل" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_messages=False)
       await event.reply("- تم قفل الرسائل")
    elif "الميديا" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_media=False)
       await event.reply("- تم قفل الوسائط")
    elif "الملصقات" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_stickers=False)
       await event.reply("- تم قفل الملصقات.")
    elif "المتحركة"in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_gifs=False)
       await event.reply("- تم قفل المتحركة")
    elif "التوجيه" in input_str:
       await DRAGON.edit_permissions(event.chat_id, forwards=False)
       await event.reply("- تم قفل التوجيه")
    elif "الالعاب" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_games=False)
       await event.reply("- تم قفل الالعاب")
    elif "الانلاين" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_inline=False)
       await event.reply("- تم قفل الانلاين")
    elif "التصويت" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_polls=False)
       await event.reply("- تم قفل التصويت")
    elif "الروابط" in input_str:
       await DRAGON.edit_permissions(event.chat_id, embed_link_previews=False)
       await event.reply("- تم قفل الروابط")
    elif "الكل" in input_str:
       await DRAGON.edit_permissions(event.chat_id,
          send_messages=False, 
          send_media=False,
          send_stickers=False,
          send_gifs=False,
...
async def unlock(event, perm):
    ...
    if not input_str:
       ...
       return
    if "الرسائل" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_messages=True)
       await event.reply("تم فتح الكتابة")
    elif "الميديا" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_media=True)
       await event.reply("تم فتح الوسائط")
    elif "الملصقات" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_stickers=True)
       await event.reply("تم فتح الملصقات")
    elif "المتحركة"in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_gifs=True)
       await event.reply("تم فتح المتحركة")
    elif "التوجيه" in input_str:
       await DRAGON.edit_permissions(event.chat_id, forwards=True)
       await event.reply("تم فتح التوجيه")
    elif "الالعاب" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_games=True)
       await event.reply("تم فتح الالعاب")
    elif "الانلاين" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_inline=True)
       await event.reply("تم فتح الانلاين")
    elif "التصويت" in input_str:
       await DRAGON.edit_permissions(event.chat_id, send_polls=True)
       await event.reply("تم فتح `التصويت")
    elif "الروابط" in input_str:
       await DRAGON.edit_permissions(event.chat_id, embed_link_previews=True)
...
@DRAGON.on(events.callbackquery.CallbackQuery(data="locks"))
async def _(event):
